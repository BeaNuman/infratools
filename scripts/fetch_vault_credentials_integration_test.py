...
# Source the script with a stub Vault server and check that the secret data is available as env vars
def test_variables_are_sourced_correctly(httpserver):
    httpserver.expect_request("/v1/auth/approle/login").respond_with_json({"auth": {"client_token": "xyz-789"}})
    httpserver.expect_request(f"/v1/{VAULT_ENV_VARS['VAULT_KV_STORE']}/data/{VAULT_ENV_VARS['VAULT_SECRET']}").respond_with_json({"data": {"data": {"KEY1": "VALUE1", "KEY2": "VALUE2"}}})
    env = {'VAULT_ADDRESS': httpserver.url_for('')} | VAULT_ENV_VARS

    stdout = subprocess.run(
        [f'source {os.path.dirname(__file__)}/fetch_vault_credentials.sh && echo "KEY1 value is $KEY1, KEY2 value is $KEY2"'],
        check=True,
        shell=True,
        capture_output=True,
        env=env
